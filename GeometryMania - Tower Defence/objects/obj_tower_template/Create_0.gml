...
function set_sequence() {
	...
	switch (state) {
		case TOWER_STATES.IDLE: {
			...
			if (_choice >= 30) {
				tower_seq_idle = layer_sequence_create("Instances", x, y, seq_tower_archer_idle_1)
			} else if (_choice >= 10 and _choice < 30) {
				tower_seq_idle = layer_sequence_create("Instances", x, y, seq_tower_archer_idle_2)
			} else {
				tower_seq_idle = layer_sequence_create("Instances", x, y, seq_tower_archer_idle_3)
			}
		...
		case TOWER_STATES.ATACK: {
			tower_seq_attack = layer_sequence_create("Instances", x, y, seq_tower_archer_atack)
...
tower_type = TOWER_TYPES.TOWER_DISTANT
tower = TOWER_INFO.TOWER_ARCHER
index = UNIT_INFO.T_ARCHER
